        ...
        <div class="card-body">
            <form action="{{ route('admin.facilities.update', $facility->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="branch_id" class="form-select" required>
                        ...
                        @foreach ($branches as $branch)
                            <option value="{{ $branch->id }}" {{ $branch->id == $facility->branch_id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control" rows="4" placeholder="Enter description (optional)">{{ old('description', $facility->description) }}</textarea>
                    @error('description')
                        <small class="text-danger d-block mt-1">{{ $message }}</small>
                    @enderror
                </div>
