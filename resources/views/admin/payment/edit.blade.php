...
@section('content')
    <h4>{{ __('admin.general.edit_payment') }}</h4>

    <form action="{{ route('admin.payments.update', ['locale' => app()->getLocale(), 'id' => $payment->id]) }}" method="POST" class="row g-3">
        @csrf
        @method('PUT')
        <div class="col-md-6">
            ...
            <select name="reservation_id" class="form-select">
                ...
                @foreach($reservations as $reservation)
                    <option value="{{ $reservation->id }}" @selected(old('reservation_id', $payment->reservation_id)==$reservation->id)>
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="payment_method" value="{{ old('payment_method', $payment->payment_method) }}" class="form-control" required>
        </div>
        <div class="col-md-6">
            <label class="form-label">{{ __('admin.forms.payment_method_ar') }}</label>
            <input type="text" name="payment_method_ar" value="{{ old('payment_method_ar', $payment->payment_method_ar ?? '') }}" class="form-control" dir="rtl">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <select name="status" class="form-select">
                @foreach(['paid','pending','failed'] as $status)
                    <option value="{{ $status }}" @selected(old('status', $payment->status)==$status)>{{ __('admin.payment_statuses.' . $status) }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-4">
            <label class="form-label">{{ __('admin.general.reference') }}</label>
            <input type="text" name="reference" value="{{ old('reference', $payment->reference) }}" class="form-control" placeholder="{{ __('admin.forms.receipt_reference') }}">
        </div>
        <div class="col-12 d-flex gap-2">
            <button type="submit" class="btn btn-primary">{{ __('admin.general.update_payment') }}</button>
